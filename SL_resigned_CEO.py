...
for article in data:
    ...
    for p in person:
        for o in organisation:
            try:
                if {
                     p['surface-form'].lower() in current_article['old_ceo']['surface-form'].lower() and
            ...
            if amount_of_data < num_train:
                try:
                    if p == current_article['old_ceo'] and o == current_article['org_appointed_new_ceo']:
                        data_label.append(1)
                        ...
                        per.append(current_article['old_ceo']['surface-form'])
...
for i in xrange(len(X_test)):
    if result['class'].iloc[i] == 1:
        ...
        try:
            print '=' * 100
            print df_test['PER'].iloc[i]['surface-form']
            print df_test['ORG'].iloc[i]['surface-form']
            print '=' * 30
            print df_test['per'].iloc[i]
            print df_test['org'].iloc[i]
            print '*' * 30
            if (
